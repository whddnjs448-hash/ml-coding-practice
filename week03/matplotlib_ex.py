...
#상관 행렬 계산
correlation_matrix = titanic.drop('Passengerld', axis=1).corr(numeeric_only=True)
...
## **영역 채우기 그래프 : 나이대별 생존자와 사망자 수 표한하기**
# 결측치 처리
titanic = titanic.dropna(subset=['Age','Fare'])
...
# x,y값 설정
x = parch_counts.index.astype(str) # 데이터프레임의 인덱스르 문자열로 변환
y1 = parch_counts[0].values # 사망자
y2 = parch_counts[1].values # 생존자
...
# 첫 번쨰 그래프 설정 (선 그래프)
ax1.plot(x, y1, '-s', color='indigo', markesize=7, linewidth=5, alpha=0.7, label='Not Survived')
ax1.set_xlabel('Parch') # x축의 레이블
ax1.set_ylabel('Not Survived Count', color=)